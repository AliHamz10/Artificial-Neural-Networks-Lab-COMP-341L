"""
Convert Assignment1_Report.md to PDF with images embedded.
Run from repo root: python "Assignments/Zarmeena Jawad's Assignments/md_to_pdf.py"
...
# Script directory = Zarmeena's assignment folder; all paths (MD, HTML, figures) are under this folder
BASE = os.path.abspath(os.path.dirname(os.path.abspath(__file__)))
FIGURES_DIR = os.path.join(BASE, "figures")
MD_PATH = os.path.join(BASE, "Assignment1_Report.md")
HTML_PATH = os.path.join(BASE, "Assignment1_Report.html")
PDF_PATH = os.path.join(BASE, "Assignment1_Report.pdf")

# Title page details for submission
TITLE_PAGE = {
    "name": "Zarmeena Jawad",
    "registration_number": "B23F0115AI125",
    "course": "Artificial Neural Networks (ANN)",
    "professor": "Dr. Abid Ali",
    "assignment_title": "Assignment 1",
    "subtitle": "Foundational Models: Perceptron, Adaline, and XOR",
    "due_date": "21-02-2026",
}
...
def main():
    ...
    # Embed images from THIS assignment's figures/ folder only (Zarmeena's figures)
    def embed_images(html):
        def repl(m):
            ...
            if not src.startswith("figures/"):
                return full
            # Resolve path under this assignment's figures directory only
            filename = os.path.basename(src)
            path = os.path.join(FIGURES_DIR, filename)
            if not os.path.isfile(path):
                print("Warning: figure not found (using Zarmeena's figures/):", path)
                return full
